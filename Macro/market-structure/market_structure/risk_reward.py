"""
Per-ticker risk:reward analysis — orchestrates indicators, pivots, the six
...
pipeline order -- plus the trend-violation refinement this toolkit adds on
top (trend_violation.py), which the original pipeline has no equivalent
step for. Pure math, no I/O: caller supplies OHLCV arrays already fetched
and quality-gated (see market_data.py).
"""
...
from market_structure.fib_extension import FibExtension, build_fib_extension
...
from market_structure.levels import (
    ConvergenceCluster,
    build_anchored_vwap,
    build_channels,
...
from market_structure.stop_target import StopTarget, compute_stop_target
from market_structure.trend_violation import TrendViolation, detect_trend_violation
from market_structure.trendlines import (
    RegressionTrendline,
    TrendlineSegment,
...
def analyze(
    ...
    """Run the full pipeline (rr.txt Part 12, steps 1-9) for one ticker.
