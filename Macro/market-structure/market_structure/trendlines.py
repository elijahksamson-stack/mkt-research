...
detector (trend_violation.py) is built on, since it is the closer analogue
of "the rising support line someone drew by hand."

Pure math, no I/O — a verbatim port of RiskRewardAnalyzer's
_quantile_regression / _build_regression_trendlines /
_build_multi_lookback_trendlines.
"""
...
from market_structure.indicators import Pivot, find_local_extrema
from market_structure.levels import Level
...
def build_pivot_pair_trendlines(
    ...
    `max_lines` (rr.txt's own PAIR_MAX_LINES=12 by default, matching the
    source spec for the chart-overlay/tier-2-target use case). Pass
